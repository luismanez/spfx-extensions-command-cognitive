...
export default class SentimentAnalyticsFieldCustomizer
  ...
  public onRenderCell(event: IFieldCustomizerCellEventParameters): void {
    ...
    //const text: string = `${this.properties.sampleText}: ${id} - ${title}`;
    

    const sentimentAnalytics: React.ReactElement<{}> =
      React.createElement(SentimentAnalytics, { id: id, text: title, httpClient: this.context.httpClient } as ISentimentAnalyticsProps);
